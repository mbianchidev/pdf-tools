...
@Component
public class PdfToPowerPointEngine {

    private static final Logger logger =
        LoggerFactory.getLogger(PdfToPowerPointEngine.class);

    private final PdfToPowerPointProperties properties;

    public PdfToPowerPointEngine(PdfToPowerPointProperties properties) {
    ...
    public Path convert(
        ...
        Path output = workspace.resolve("pdf-to-powerpoint.pptx");
        Path request = workspace.resolve(".pdf-powerpoint-request.bin");
        Path progressFile = workspace.resolve(".pdf-powerpoint-progress");
        Path errorFile = workspace.resolve(".pdf-powerpoint-error");
        PdfToPowerPointRequest.write(
            request,
            source,
            output,
            workspace,
        ...
        if (exitCode != 0) {
            RuntimeException failure = IsolatedJavaWorker.readFailure(
                exitCode,
                errorFile,
                "PDF_POWERPOINT_RESOURCE_LIMIT_EXCEEDED",
                "The isolated PDF-to-PowerPoint worker exited early",
                logger
            );
            deleteOutput(output, failure);
            throw failure;
        }
        validatePptx(output, pageCount);
        progress.accept(95);
        return output;
    }
    ...
    private int inspect(
        ...
        Path scratch = workspace.resolve(".pdf-powerpoint-inspection");
        try {
            Files.createDirectories(scratch);
        } catch (IOException exception) {
            throw new OperationException(
                "PDF_POWERPOINT_SCRATCH_FAILED",
                "PDF-to-PowerPoint inspection scratch could not be created",
                exception
            );
        }
    ...
    private IsolatedJavaWorker.Spec workerSpec() {
        return new IsolatedJavaWorker.Spec(
            PdfToPowerPointWorkerMain.class,
            properties.getWorkerHeapBytes(),
            properties.getWorkerTimeout(),
            "PDF_POWERPOINT_WORKER_START_FAILED",
            "The isolated PDF-to-PowerPoint worker could not be started",
            "PDF_POWERPOINT_TIMEOUT",
            "PDF-to-PowerPoint conversion exceeded its time limit"
        );
    }

    private void validatePptx(Path output, int expectedSlides) {
        try {
            ...
            try (InputStream file = Files.newInputStream(output);
                ...
                while ((entry = archive.getNextEntry()) != null) {
                    ...
                    presentation |= name.equals("ppt/presentation.xml");
    ...
    private void deleteOutput(Path output, RuntimeException failure) {
        try {
            ...
            logger.error(
                "Could not remove partial PDF-to-PowerPoint output {}",
                output,
                exception
            );
        }
    ...
    private OperationException protocolFailure(Throwable cause) {
        return new OperationException(
            ...
            "The PDF-to-PowerPoint worker returned invalid state",
            cause
        );
    }
    ...
    private OperationException invalidOutput(Throwable cause) {
        return new OperationException(
            "INVALID_PDF_POWERPOINT_OUTPUT",
            "The generated PowerPoint presentation is invalid",
            cause
        );
    }
