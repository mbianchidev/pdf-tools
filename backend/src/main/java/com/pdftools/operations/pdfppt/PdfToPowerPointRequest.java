...
record PdfToPowerPointRequest(
    ...
    static void write(
        ...
        try (DataOutputStream data = new DataOutputStream(
            ...
            data.writeInt(properties.getMaxTextBoxes());
    ...
    static PdfToPowerPointRequest read(Path path) {
        try (DataInputStream data = new DataInputStream(
            ...
            var plan = new PdfToPowerPointPlanFactory.PdfToPowerPointPlan(
                data.readUTF(),
                data.readUTF(),
                data.readBoolean(),
                data.readBoolean()
            ...
            properties.setMaxTextBoxes(data.readInt());
            ...
            if (data.read() != -1
                    ...
                    || !valid(plan, properties)) {
                throw protocolFailure(null);
            }
            return new PdfToPowerPointRequest(
                source,
                output,
                workspace,
                plan,
    ...
    private static boolean valid(
            PdfToPowerPointPlanFactory.PdfToPowerPointPlan plan,
            PdfToPowerPointProperties properties) {
        return (plan.mode().equals("editable")
                || plan.mode().equals("visual"))
            && (plan.slideSize().equals("source")
                || plan.slideSize().equals("widescreen")
                || plan.slideSize().equals("standard"))
            && properties.getMaxPages() > 0
            && properties.getMaxTextCharacters() > 0
            && properties.getMaxTextBoxes() > 0
    ...
    private static OperationException protocolFailure(Throwable cause) {
        return new OperationException(
            ...
            "The PDF-to-PowerPoint worker received invalid state",
            cause
        );
    }
}
