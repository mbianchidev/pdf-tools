...
@Component
public class PdfToExcelOperation implements PdfOperation {
    ...
    private final PdfToExcelEngine engine;
    private final PdfToExcelPlanFactory planFactory;
    private final PdfToExcelProperties properties;

    public PdfToExcelOperation(
            PdfToExcelEngine engine,
            PdfToExcelPlanFactory planFactory,
    ...
    public void validateSubmission(OperationSubmission submission) {
        if (submission.files().size() != 1) {
            throw new OperationException(
                "INVALID_FILE_COUNT",
                "PDF to Excel requires exactly one PDF"
            );
        }
        ...
        if (!file.filename().toLowerCase(Locale.ROOT).endsWith(".pdf")
            ...
            throw new OperationException(
                "INVALID_PDF_FILE",
                "PDF to Excel accepts one PDF file"
            );
        }
        if (file.sizeBytes() < 1
