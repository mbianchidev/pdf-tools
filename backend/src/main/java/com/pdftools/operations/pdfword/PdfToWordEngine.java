...
@Component
public class PdfToWordEngine {

    private static final Logger logger =
        LoggerFactory.getLogger(PdfToWordEngine.class);

    private final PdfToWordProperties properties;

    public PdfToWordEngine(PdfToWordProperties properties) {
    ...
    public Path convert(
        ...
        Path output = workspace.resolve("pdf-to-word.docx");
        Path request = workspace.resolve(".pdf-word-request.bin");
        Path progressFile = workspace.resolve(".pdf-word-progress");
        Path errorFile = workspace.resolve(".pdf-word-error");
        PdfToWordRequest.write(
            request,
            source,
            output,
            workspace,
        ...
        if (exitCode != 0) {
            RuntimeException failure = IsolatedJavaWorker.readFailure(
                exitCode,
                errorFile,
                "PDF_WORD_RESOURCE_LIMIT_EXCEEDED",
                "The isolated PDF-to-Word worker exited before completing",
                logger
            );
    ...
    private int inspect(
        ...
        Path scratch = workspace.resolve(".pdf-word-inspection");
        try {
            Files.createDirectories(scratch);
        } catch (IOException exception) {
            throw new OperationException(
                "PDF_WORD_SCRATCH_FAILED",
                "PDF-to-Word inspection scratch could not be created",
                exception
            );
        }
    ...
    private IsolatedJavaWorker.Spec workerSpec() {
        return new IsolatedJavaWorker.Spec(
            PdfToWordWorkerMain.class,
            properties.getWorkerHeapBytes(),
            properties.getWorkerTimeout(),
            "PDF_WORD_WORKER_START_FAILED",
            "The isolated PDF-to-Word worker could not be started",
            "PDF_WORD_TIMEOUT",
            "PDF-to-Word conversion exceeded its time limit"
        );
    }

    private void validateDocx(Path output) {
        try {
            ...
            try (InputStream file = Files.newInputStream(output);
                ...
                while ((entry = archive.getNextEntry()) != null) {
                    ...
                    contentTypes |= entry.getName().equals(
                        "[Content_Types].xml"
                    );
                    documentXml |= entry.getName().equals(
                        "word/document.xml"
                    );
                    int read;
    ...
    private void deleteOutput(Path output, RuntimeException failure) {
        try {
            ...
            logger.error(
                "Could not remove partial PDF-to-Word output {}",
                output,
                exception
            );
        }
    ...
    private OperationException protocolFailure(Throwable cause) {
        return new OperationException(
            ...
            "The PDF-to-Word worker returned invalid state",
            cause
        );
    }
    ...
    private OperationException invalidOutput(Throwable cause) {
        return new OperationException(
            "INVALID_PDF_WORD_OUTPUT",
            "The generated Word document is invalid",
            cause
        );
    }
