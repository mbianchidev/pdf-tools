...
@SpringBootTest(properties = "pdf.jobs.enabled-operations=watermark")
class WatermarkJobIntegrationTest {
    ...
    @Test
    void watermarksPdfThroughPersistentJob() throws Exception {
    ...
    private JobResponse awaitTerminal(JobResponse created) throws Exception {
        ...
        if (!current.status().isTerminal()) {
            throw new AssertionError("Watermark job did not complete");
        }
        return current;
    }
}
