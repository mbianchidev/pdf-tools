...
@SpringBootTest(properties = "pdf.jobs.enabled-operations=rotate")
class RotateJobIntegrationTest {
    ...
    @Test
    void rotatesSelectedPagesThroughPersistentJobPipeline() throws Exception {
        ...
        try (StoredResource resource = storageService.get(
            ...
            assertEquals(0, document.getPage(0).getRotation());
    ...
    private JobResponse awaitTerminal(JobResponse created) throws Exception {
        ...
        if (!current.status().isTerminal()) {
            throw new AssertionError("Rotate PDF job did not complete");
        }
        return current;
    }
}
