...
@SpringBootTest(properties = "pdf.jobs.enabled-operations=organize")
class OrganizeJobIntegrationTest {
    ...
    @Test
    void organizesPagesThroughPersistentJobPipeline() throws Exception {
        ...
        try (StoredResource resource = storageService.get(
            ...
            assertEquals(300, document.getPage(0).getMediaBox().getWidth());
            assertEquals(100, document.getPage(1).getMediaBox().getWidth());
            assertEquals(100, document.getPage(2).getMediaBox().getWidth());
    ...
    private JobResponse awaitTerminal(JobResponse created) throws Exception {
        ...
        if (!current.status().isTerminal()) {
            throw new AssertionError("Organize PDF job did not complete");
        }
        return current;
    }
}
